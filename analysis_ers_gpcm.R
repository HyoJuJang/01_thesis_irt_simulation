...
model{
  for(i in 1:n){
    ...
    res_om[i] <- omega[i] - m[i]
    ome.new[i]~dlnorm(m[i],sigmasq)
    res.new_om[i] <- ome.new[i] - m[i]
  ...
  for(i in 1:n){
    for(j in 1:p){
      for(k in 1:K[j]) {
        eta[i, j, k] <- alpha[j] * (theta[i] - (b[j]+omega[i]*tau[j, k]))
...
", con=modfile)
  ...
  param <- c('alpha','b','b10','b11','b20','b21','tau','psi','mu','m','theta','omega','fit','fit.new','fit_om','fit.new_om')
  inits <- function(){list(omega=rep(1, n), alpha=rep(1, p), b=rep(0, p), tau=matrix(data=(c(rep(NA, p),rep(0, 3*p),rep(NA, p))),nrow=p,ncol=5))}
